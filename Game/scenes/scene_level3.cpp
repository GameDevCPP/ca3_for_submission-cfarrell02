...
void Level3Scene::Load() {
    ...
    ls::loadLevelFile("res/level3.json", 40.0f);

    _levelMusic = Resources::get<sf::SoundBuffer>("Final Music.wav");
  ...
  // Create player
    ...
    // First particle generator
        ...
        int spread[] = {100,100,200,200 ,200,200 ,   100};
    ...
    if (platforms.size() % 2 == 0 && !platforms.empty()) {
        ...
        generatePlatforms(platforms, 5, 4);
        generatePlatforms(platforms, 3, 6);
        generatePlatforms(platforms, 2, 8);
    }
...
//        pos = {pos.x + 40, pos.y + 40};
...

    {
        ...
        auto endTexture = Resources::get<Texture>("Free/Items/Checkpoints/End/End (Idle).png");
        auto esprite = flag->addComponent<SpriteComponent>();
        esprite->setTexture(endTexture);
        esprite->getSprite().setOrigin(32.f, 32.f);
        flag->addComponent<NextLevelComponent >(player,std::make_shared<DeathScene>(), true);
    }
    ...
    for(int x = 0; x < width; x+=spriteSize) {
        for(int y = 0; y < height;  y+=spriteSize) {
            ...
            sprite.setTexture(*Resources::get<Texture>("Free/Background/Green.png"));
...
void Level3Scene::generatePlatforms(std::vector<sf::Vector2ul> &platforms, int amount, int startIndex) {
